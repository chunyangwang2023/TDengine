...
extern bool mcIsInit();

int32_t mcCheckQuery(SParseContext *pParseCxt, SQuery *pQuery) {
  if (!mcIsInit()) return 0;

  SNode    *pNode = pQuery->pRoot;
  ENodeType nt = nodeType(pNode);

#define SUPPORT_TYPE(type) \
  if (nt == type) return 0;

#define UNSUPPORT_TYPE(type)            \
  if (nt == type) {                     \
    terrno = TSDB_CODE_OPS_NOT_SUPPORT; \
    return terrno;                      \
  }
  ...
  SUPPORT_TYPE(QUERY_NODE_COLUMN);
  SUPPORT_TYPE(QUERY_NODE_VALUE);
  SUPPORT_TYPE(QUERY_NODE_OPERATOR);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_CONDITION);
  SUPPORT_TYPE(QUERY_NODE_FUNCTION);
  SUPPORT_TYPE(QUERY_NODE_REAL_TABLE);
  UNSUPPORT_TYPE(QUERY_NODE_TEMP_TABLE);
  UNSUPPORT_TYPE(QUERY_NODE_JOIN_TABLE);
  UNSUPPORT_TYPE(QUERY_NODE_GROUPING_SET);
  SUPPORT_TYPE(QUERY_NODE_ORDER_BY_EXPR);
  SUPPORT_TYPE(QUERY_NODE_LIMIT);
  UNSUPPORT_TYPE(QUERY_NODE_STATE_WINDOW);
  UNSUPPORT_TYPE(QUERY_NODE_SESSION_WINDOW);
  UNSUPPORT_TYPE(QUERY_NODE_INTERVAL_WINDOW);
  SUPPORT_TYPE(QUERY_NODE_NODE_LIST);
  UNSUPPORT_TYPE(QUERY_NODE_FILL);
  SUPPORT_TYPE(QUERY_NODE_RAW_EXPR);  // Only be used in parser module.
  SUPPORT_TYPE(QUERY_NODE_TARGET);
  SUPPORT_TYPE(QUERY_NODE_DATABLOCK_DESC);
  SUPPORT_TYPE(QUERY_NODE_SLOT_DESC);
  SUPPORT_TYPE(QUERY_NODE_COLUMN_DEF);
  SUPPORT_TYPE(QUERY_NODE_DOWNSTREAM_SOURCE);
  SUPPORT_TYPE(QUERY_NODE_DATABASE_OPTIONS);
  SUPPORT_TYPE(QUERY_NODE_TABLE_OPTIONS);
  UNSUPPORT_TYPE(QUERY_NODE_INDEX_OPTIONS);
  UNSUPPORT_TYPE(QUERY_NODE_EXPLAIN_OPTIONS);
  UNSUPPORT_TYPE(QUERY_NODE_STREAM_OPTIONS);
  SUPPORT_TYPE(QUERY_NODE_LEFT_VALUE);
  SUPPORT_TYPE(QUERY_NODE_COLUMN_REF);
  UNSUPPORT_TYPE(QUERY_NODE_WHEN_THEN);
  UNSUPPORT_TYPE(QUERY_NODE_CASE_WHEN);
  UNSUPPORT_TYPE(QUERY_NODE_EVENT_WINDOW);

  // Statement nodes are used in parser and planner module.
  SUPPORT_TYPE(QUERY_NODE_SET_OPERATOR);
  SUPPORT_TYPE(QUERY_NODE_SELECT_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_VNODE_MODIFY_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_DATABASE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_DATABASE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_ALTER_DATABASE_STMT);
  SUPPORT_TYPE(QUERY_NODE_FLUSH_DATABASE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_TRIM_DATABASE_STMT);
  SUPPORT_TYPE(QUERY_NODE_CREATE_TABLE_STMT);
  SUPPORT_TYPE(QUERY_NODE_CREATE_SUBTABLE_CLAUSE);
  SUPPORT_TYPE(QUERY_NODE_CREATE_MULTI_TABLES_STMT);
  SUPPORT_TYPE(QUERY_NODE_DROP_TABLE_CLAUSE);
  SUPPORT_TYPE(QUERY_NODE_DROP_TABLE_STMT);
  SUPPORT_TYPE(QUERY_NODE_DROP_SUPER_TABLE_STMT);
  SUPPORT_TYPE(QUERY_NODE_ALTER_TABLE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_ALTER_SUPER_TABLE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_USER_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_ALTER_USER_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_USER_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_USE_DATABASE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_DNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_DNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_ALTER_DNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_INDEX_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_INDEX_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_QNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_QNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_BNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_BNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_SNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_SNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_MNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_MNODE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_TOPIC_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_TOPIC_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_CGROUP_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_ALTER_LOCAL_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_EXPLAIN_STMT);
  SUPPORT_TYPE(QUERY_NODE_DESCRIBE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_RESET_QUERY_CACHE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_COMPACT_DATABASE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_FUNCTION_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_FUNCTION_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_CREATE_STREAM_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DROP_STREAM_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_BALANCE_VGROUP_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_MERGE_VGROUP_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_REDISTRIBUTE_VGROUP_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SPLIT_VGROUP_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SYNCDB_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_GRANT_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_REVOKE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_DNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_MNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_MODULES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_QNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_SNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_BNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_CLUSTER_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_DATABASES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_FUNCTIONS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_INDEXES_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_STABLES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_STREAMS_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_TABLES_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_TAGS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_USERS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_LICENCES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_VGROUPS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_TOPICS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_CONSUMERS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_CONNECTIONS_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_QUERIES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_APPS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_VARIABLES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_DNODE_VARIABLES_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_TRANSACTIONS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_SUBSCRIPTIONS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_VNODES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_USER_PRIVILEGES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_CREATE_DATABASE_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_CREATE_TABLE_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_CREATE_STABLE_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_TABLE_DISTRIBUTED_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_LOCAL_VARIABLES_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_SCORES_STMT);
  SUPPORT_TYPE(QUERY_NODE_SHOW_TABLE_TAGS_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_KILL_CONNECTION_STMT);
  SUPPORT_TYPE(QUERY_NODE_KILL_QUERY_STMT);
  SUPPORT_TYPE(QUERY_NODE_KILL_TRANSACTION_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_DELETE_STMT);
  SUPPORT_TYPE(QUERY_NODE_INSERT_STMT);
  SUPPORT_TYPE(QUERY_NODE_QUERY);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_DB_ALIVE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_SHOW_CLUSTER_ALIVE_STMT);
  UNSUPPORT_TYPE(QUERY_NODE_BALANCE_VGROUP_LEADER_STMT);

  // logic plan node
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_SCAN);
  UNSUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_JOIN);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_AGG);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_PROJECT);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_VNODE_MODIFY);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_EXCHANGE);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_MERGE);
  UNSUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_WINDOW);
  UNSUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_FILL);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_SORT);
  UNSUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_PARTITION);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_INDEF_ROWS_FUNC);
  UNSUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN_INTERP_FUNC);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_SUBPLAN);
  SUPPORT_TYPE(QUERY_NODE_LOGIC_PLAN);

  // physical plan node
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_TAG_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_TABLE_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_TABLE_SEQ_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_TABLE_MERGE_SCAN);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_SYSTABLE_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_BLOCK_DIST_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_LAST_ROW_SCAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_PROJECT);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_JOIN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_HASH_AGG);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_EXCHANGE);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_SORT);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_GROUP_SORT);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_HASH_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_ALIGNED_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_FINAL_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_SEMI_INTERVAL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_FILL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_FILL);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_SESSION);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_SESSION);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_SEMI_SESSION);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_FINAL_SESSION);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_STATE);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_STATE);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_PARTITION);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_PARTITION);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_INDEF_ROWS_FUNC);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_INTERP_FUNC);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_DISPATCH);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_INSERT);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_QUERY_INSERT);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_DELETE);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_SUBPLAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN);
  SUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_TABLE_COUNT_SCAN);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_MERGE_EVENT);
  UNSUPPORT_TYPE(QUERY_NODE_PHYSICAL_PLAN_STREAM_EVENT);
...
int32_t mcCheckCol(int32_t type) {
  ...
  if (type == TSDB_DATA_TYPE_BOOL || type == TSDB_DATA_TYPE_TIMESTAMP || type == TSDB_DATA_TYPE_INT ||
      type == TSDB_DATA_TYPE_UINT || type == TSDB_DATA_TYPE_BIGINT || type == TSDB_DATA_TYPE_UBIGINT ||
      type == TSDB_DATA_TYPE_VARCHAR || type == TSDB_DATA_TYPE_FLOAT || type == TSDB_DATA_TYPE_DOUBLE ||
      type == TSDB_DATA_TYPE_SMALLINT || type == TSDB_DATA_TYPE_USMALLINT || type == TSDB_DATA_TYPE_TINYINT ||
...
int32_t mcCheckDbTable(const char *db, const char *table) {
  ...
  if (strcasecmp(db, TSDB_INFORMATION_SCHEMA_DB) == 0) {
    if (strcasecmp(table, TSDB_INS_TABLE_DNODES) == 0 || strcasecmp(table, TSDB_INS_TABLE_MNODES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_MODULES) == 0 || strcasecmp(table, TSDB_INS_TABLE_QNODES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_BNODES) == 0 || strcasecmp(table, TSDB_INS_TABLE_SNODES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_CLUSTER) == 0 || strcasecmp(table, TSDB_INS_TABLE_DATABASES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_FUNCTIONS) == 0 || strcasecmp(table, TSDB_INS_TABLE_INDEXES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_USERS) == 0 || strcasecmp(table, TSDB_INS_TABLE_LICENCES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_VGROUPS) == 0 || strcasecmp(table, TSDB_INS_TABLE_VNODES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_CONFIGS) == 0 || strcasecmp(table, TSDB_INS_TABLE_DNODE_VARIABLES) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_SUBSCRIPTIONS) == 0 || strcasecmp(table, TSDB_INS_TABLE_TOPICS) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_STREAMS) == 0 || strcasecmp(table, TSDB_INS_TABLE_STREAM_TASKS) == 0 ||
        strcasecmp(table, TSDB_INS_TABLE_USER_PRIVILEGES) == 0) {
      terrno = TSDB_CODE_OPS_NOT_SUPPORT;
      return terrno;
    }
  }

  if (strcasecmp(db, TSDB_PERFORMANCE_SCHEMA_DB) == 0) {
    terrno = TSDB_CODE_OPS_NOT_SUPPORT;
    return terrno;
  }
